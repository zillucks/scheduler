                    ...
                    <div class="card">
                        <div class="card-header bg-white clearfix">
                            ...
                            <div class="float-right">
                                <a href="{{ route('reservations.booking-wizard', 'step=2') }}" class="btn btn-sm btn-danger"><i class="fas fa-arrow-left"></i> Back</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('reservations.form-wizard.step-3') }}" method="POST" id="form-booking-confirm">
                                @csrf
                                <p>{{ $user->identity->full_name }}, Silahkan melengkapi sesi training yang anda pilih pada
                                    form dibawah ini</p>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select name="reservation_user[]" id="reservation_user" size="5" multiple required>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-4 col-md-3">
                                        <input type="date" name="reservation_date" id="reservation_date" class="form-control {{ $errors->has('reservation_date') ? 'is-invalid' : '' }}"
                                            value="{{ old('reservation_date') }}" aria-describedby="available-quotes" required>
                                        @if ($errors->has('reservation_date'))
                                            <div class="invalid-feedback">{{ $errors->first('reservation_date') }}</div>
                                        @endif
                                        <div class="form-text text-info" id="available-quotes"></div>
                                    </div>
                                    <div class="col-sm-4 col-md-3">
                ...
                if (reservation_date.val() == '') {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $('#available-quotes').html('Sisa Quota : ' + response.data.available_quote);
